...
const App: React.FC = () => {
  ...
  // ESCUTA EM TEMPO REAL: Garante que PC e Celular vejam a mesma coisa
  useEffect(() => {
    const unsubs = [
  ...
  const groupedMenu = useMemo(() => {
    ...
    if (selectedCategory === 'Todos') return [{ title: 'Nosso Cardápio', items: filtered }];
  ...
  return (
    <div className="min-h-screen bg-slate-50 flex flex-col w-full overflow-x-hidden">
      ...
      {/* BANNER LOJA FECHADA - IMPACTANTE E EM TEMPO REAL */}
      {!isAdmin && !isStoreOpen && (
        <div className="w-full bg-red-600 py-4 flex items-center justify-center animate-pulse shadow-lg z-40 sticky top-20 sm:top-28">
           <span className="text-white font-black uppercase text-[10px] sm:text-xs tracking-[0.2em] text-center px-4">
             🚫 Loja Fechada Agora. Voltamos em breve! 🚫
           </span>
        </div>
      )}
        ...
        {isAdmin ? (
          <AdminPanel 
            ...
            onToggleStore={async () => { 
              ...
              await dbService.save('settings', 'general', { id: 'general', isStoreOpen: newVal, logoUrl }); 
            }} 
            logoUrl={logoUrl} onUpdateLogo={async (url) => { 
              await dbService.save('settings', 'general', { id: 'general', isStoreOpen, logoUrl: url }); 
            }}
            onAddProduct={async (p) => { const id = Date.now().toString(); await dbService.save('products', id, {...p, id} as Product); }} 
            onUpdateProduct={async (p) => { await dbService.save('products', p.id, p); }} 
            onDeleteProduct={async (id) => { await dbService.remove('products', id); }}
            onUpdateOrderStatus={async (id, s) => { const order = orders.find(o => o.id === id); if (order) await dbService.save('orders', id, { ...order, status: s }); }} 
            onUpdateCustomer={async (id, u) => { const c = customers.find(x => x.id === id); if(c) await dbService.save('customers', id, {...c, ...u}); }}
            onAddCategory={async (n) => { const id = Date.now().toString(); await dbService.save('categories', id, {id, name: n}); }}
            onRemoveCategory={async (id) => { await dbService.remove('categories', id); }}
            onAddSubCategory={async (catId, n) => { const id = Date.now().toString(); await dbService.save('sub_categories', id, {id, categoryId: catId, name: n}); }}
            onRemoveSubCategory={async (id) => { await dbService.remove('sub_categories', id); }}
            onAddComplement={async (n, p, c) => { const id = Date.now().toString(); await dbService.save('complements', id, {id, name: n, price: Number(p), active: true, applicable_categories: c}); }}
            onToggleComplement={async (id) => { const comp = complements.find(c => c.id === id); if(comp) await dbService.save('complements', id, {...comp, active: !comp.active}); }}
            onRemoveComplement={async (id) => { await dbService.remove('complements', id); }}
            onAddZipRange={async (s, e, f) => { const id = Date.now().toString(); await dbService.save('zip_ranges', id, {id, start: s, end: e, fee: Number(f)}); }}
            onRemoveZipRange={async (id) => { await dbService.remove('zip_ranges', id); }}
            onAddCoupon={async (c, d, t) => { const id = Date.now().toString(); await dbService.save('coupons', id, {id, code: c.toUpperCase(), discount: Number(d), type: t, active: true}); }}
            ...
            onAddPaymentMethod={async (n, t) => { const id = Date.now().toString(); await dbService.save('payment_methods', id, {id, name: n, type: t, enabled: true}); }}
          ...
          <div className="flex flex-col w-full">
            <section className="relative w-full min-h-[500px] bg-slate-950 flex items-center justify-center overflow-hidden">
               ...
               <div className="max-w-5xl mx-auto px-6 relative z-10 w-full text-center">
                  <h1 className="font-brand text-7xl sm:text-[120px] leading-none uppercase tracking-normal text-white drop-shadow-2xl">
            ...
            <div id="menu-anchor" className="sticky top-20 z-30 bg-white/95 backdrop-blur-md shadow-sm border-b overflow-x-auto no-scrollbar py-4 px-6 flex gap-3">
               ...
               {categories.map(cat => <button key={cat.id} onClick={() => { setSelectedCategory(cat.name); setSelectedSubCategory('Todos'); }} className={`px-6 py-2.5 rounded-full text-[10px] font-black uppercase tracking-widest transition-all shrink-0 ${selectedCategory === cat.name ? 'bg-emerald-600 text-white shadow-md' : 'bg-slate-100 text-slate-400'}`}>{cat.name}</button>)}
            </div>
            
            <div className="max-w-7xl mx-auto px-6 py-12">
               {groupedMenu.map((group, idx) => (
                 <div key={idx} className="space-y-8 animate-fade-in">
                    <h2 className="text-2xl font-black text-slate-900 uppercase border-l-4 border-emerald-600 pl-4">{group.title}</h2>
      ...
      {!isAdmin && <InstallBanner logoUrl={logoUrl} />}
    </div>
  );
};
