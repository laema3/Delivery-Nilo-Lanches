...
export const FoodCard: React.FC<FoodCardProps> = ({ product, onAdd, onClick }) => {
  return (
    <div className="group relative bg-white rounded-2xl p-4 shadow-sm hover:shadow-md transition-all border border-slate-100 cursor-pointer flex flex-row gap-4" onClick={() => onClick(product)}>
      ...
      <div className="flex-1 flex flex-col justify-between min-w-0">
        ...
        <div className="flex items-center justify-between mt-2">
          ...
          <button 
            ...
            className="bg-emerald-600 text-white px-4 py-2 rounded-xl font-black uppercase text-[10px] tracking-widest hover:bg-red-600 transition-colors shadow-md active:scale-95"
          >
            Add
          </button>
        </div>
      ...
      {/* Imagem (Direita) */}
      <div className="relative w-24 h-24 sm:w-32 sm:h-32 rounded-xl overflow-hidden shrink-0 bg-slate-50">
        {product.image ? (
